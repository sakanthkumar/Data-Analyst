#!/usr/bin/env python3
"""Bootstrap script to create services package structure."""
...
def main():
    ...
    print(f"✓ Created/verified directory: {services_dir}")
    
    # Create __init__.py
    init_py = os.path.join(services_dir, '__init__.py')
    with open(init_py, 'w') as f:
        f.write('# Services package\n')
    print(f"✓ Created: {init_py}")
    
    # Create llm_service.py
    llm_service_py = os.path.join(services_dir, 'llm_service.py')
    with open(llm_service_py, 'w') as f:
        f.write(LLM_SERVICE_CODE)
    print(f"✓ Created: {llm_service_py}")
    
    print("\n✓ Services package setup complete!")
    return 0

LLM_SERVICE_CODE = '''import subprocess
import os
